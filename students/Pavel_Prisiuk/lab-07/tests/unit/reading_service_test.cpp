...
protected:
  ...
  void SetUp() override {
    ...
    // Создаем проекцию (заглушка для тестов)
    auto proj = std::make_shared<infrastructure::projection::SensorProjectionHandler>("host=127.0.0.1 user=postgres password=password dbname=smarthome");

    // Создаем хендлеры (теперь с 3 аргументами)
    auto reg_h = std::make_shared<application::command::handlers::RegisterReadingHandler>(repo, notifier, proj);
    auto get_h = std::make_shared<application::query::handlers::GetSensorHandler>(repo);
    auto dash_h = std::make_shared<application::query::handlers::GetDashboardHandler>("host=127.0.0.1 user=postgres password=password dbname=smarthome");

    // Собираем сервис-фасад (теперь с 3 хендлерами)
